...
namespace Astar.Vanguard.Client
{
    public sealed class GameLoop : AstarVanguardSingleton<GameLoop>
    {
        ...
        void Update()
        {
            ...
            KeyInput.KeyDown(AstarVanguardPlugin.EnableLootingHotKey.Value, AstarVanguardPlugin.EnableLooting);
            KeyInput.KeyDown(AstarVanguardPlugin.EnableKeepFormationHotKey.Value, AstarVanguardPlugin.EnableKeepFormation);
        ...
        public void LoadAssetBundle()
        {
            ...
            var resourceName = "Astar.Vanguard.Client.Assets.astarvanguard";
        ...
        public void Init()
        {
            ...
            EventMgr.Subscribe<UpdateVanguardTraderAssortmentEvent>(OnUpdateVanguardTraderAssortment, this);
        }
        ...
        public void OnUpdateVanguardTraderAssortment(UpdateVanguardTraderAssortmentEvent @event)
        {
            TasksExtensions.HandleExceptions(UpdateVanguardTraderAssortmentEvent());
        }
        ...
        public async Task UpdateVanguardTraderAssortmentEvent()
        {
            foreach (var trader in Session.Traders)
            {
                if (trader.Id == AstarVanguardPlugin.VanguardTraderId)
                {
        ...
        private void BatchRefreshItems(Dictionary<ItemData, McsAILeadPlayer> updates)
        {
            ...
            foreach (var batch in batches)
            {
                foreach (var kvp in batch)
                {
                    ...
                    {
                        AstarVanguardPlugin.Logger.LogError($"Batch refresh item error: {e}");
                    }
                }
            }
        }
        ...
        public async Task InitMcsLeadPlayerConfigs()
        {
            ...
            if (AstarVanguardPlugin.FikaInstalled)
            {
        ...
        public BotDifficultySettingsClass SetBotSettings(BotDifficulty botDifficulty, WildSpawnType wildSpawnType, BotOwner botOwner, Player leadPlayer)
        {
            ...
            var operatorCodeName = botOwner.GetPlayer.Profile.Info.Nickname;
            if (botOwner.GetPlayer.Profile.Info.Side == EPlayerSide.Savage &&
                !string.IsNullOrWhiteSpace(botOwner.GetPlayer.Profile.Info.MainProfileNickname))
            {
                operatorCodeName = botOwner.GetPlayer.Profile.Info.MainProfileNickname;
            }

            var operatorProfile = VanguardOperatorCatalog.Get(operatorCodeName);
            if (
                operatorProfile != null
                && Runtime.VanguardAiAuthority.CanApplyOperatorSettings()
            )
            {
                var aim = Mathf.Clamp(operatorProfile.Aim, 0.35f, 1.25f);
                var vision = Mathf.Clamp(operatorProfile.Vision, 300f, 600f);
                var hearing = Mathf.Clamp(operatorProfile.Hearing, 55f, 140f);
                var reaction = Mathf.Clamp(operatorProfile.Reaction, 0.025f, 0.15f);
                var aggression = Mathf.Clamp(operatorProfile.Aggression, 0.55f, 1.6f);
                var damageCoeff = Mathf.Clamp(operatorProfile.DamageCoeff, 0.65f, 1.2f);
                var enemyMemory = Mathf.Clamp(operatorProfile.EnemyMemory, 20f, 100f);
                var cover = Mathf.Clamp(operatorProfile.Cover, 0.65f, 1.5f);

                settings.FileSettings.Mind.TIME_TO_FORGOR_ABOUT_ENEMY_SEC = enemyMemory;
                settings.FileSettings.Mind.TIME_TO_FIND_ENEMY = Mathf.Clamp(7f + reaction * 75f, 8f, 18f);
                settings.FileSettings.Core.VisibleDistance = vision;

                settings.FileSettings.Aiming.MAX_AIMING_UPGRADE_BY_TIME = aim;
                settings.FileSettings.Aiming.BOTTOM_COEF = aim;
                settings.FileSettings.Aiming.COEF_FROM_COVER = aim;
                settings.FileSettings.Aiming.PANIC_COEF = aim;
                settings.FileSettings.Aiming.COEF_IF_MOVE = aim;
                settings.FileSettings.Aiming.MAX_AIM_TIME = reaction;
                settings.FileSettings.Aiming.HARD_AIM = reaction;

                settings.FileSettings.Look.MINIMUM_VISIBLE_DIST = vision;
                settings.FileSettings.Look.NIGHT_VISION_ON = vision;
                settings.FileSettings.Look.NIGHT_VISION_OFF = vision;
                settings.FileSettings.Look.NIGHT_VISION_DIST = vision;
                settings.FileSettings.Look.FAR_DISTANCE = vision;
                settings.FileSettings.Look.MiddleDeltaTimeSec = reaction;
                settings.FileSettings.Look.FarDeltaTimeSec = reaction;

                settings.FileSettings.Hearing.CLOSE_DIST = hearing * 0.35f;
                settings.FileSettings.Hearing.FAR_DIST = hearing;
                settings.FileSettings.Hearing.HEAR_DELAY_WHEN_PEACE = reaction * 0.5f;
                settings.FileSettings.Hearing.HEAR_DELAY_WHEN_HAVE_SMT = reaction * 0.35f;

                settings.FileSettings.Cover.HIDE_TO_COVER_TIME = 6f / cover;
                settings.FileSettings.Cover.TIME_TO_MOVE_TO_COVER = 18f / cover;
                settings.FileSettings.Cover.HITS_TO_LEAVE_COVER = cover >= 1.2f ? 3 : cover <= 0.82f ? 1 : 2;
                settings.FileSettings.Cover.HITS_TO_LEAVE_COVER_UNKNOWN = cover >= 1.2f ? 3 : 2;
                settings.FileSettings.Cover.RETURN_TO_ATTACK_AFTER_AMBUSH_MIN = 22f / aggression;
                settings.FileSettings.Cover.RETURN_TO_ATTACK_AFTER_AMBUSH_MAX = 48f / aggression;

                botOwner.GetPlayer.ActiveHealthController.SetDamageCoeff(damageCoeff);
                botOwner.Tactic.AggressionChange(aggression);
            }
            else if (operatorProfile == null)
            {
