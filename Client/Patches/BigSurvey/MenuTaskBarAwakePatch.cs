...
using Astar.Vanguard.Client.Patches.Group;
using UnityEngine.UI;
using Astar.Vanguard.Client.Extensions;
using System;

namespace Astar.Vanguard.Client.Patches.BigSurvey;
        ...
        if (_tempNewsGameObject != null)
        {
            ...
            if (_animatedToggle != null)
            {
                _animatedToggle.onValueChanged.AddListener((arg) =>
                {
                    ...
                    stringBuilder.Append("- Mcs Version: ").Append(AstarVanguardPlugin.ClientVersion).Append("\n");
                    stringBuilder.Append("- EFT Version: ").Append(EFTVersionInfoClass.String_0).Append("\n")
                        ...
                        .Append("- Used Brain:").Append('\n').Append(string.Join(", ", AstarVanguardPlugin.LogBuffer.GetUsedBrains().Select(kvp => $"{kvp.Key}({kvp.Value}s)"))).Append('\n')
                        .Append("- Used Layer:").Append('\n').Append(string.Join(", ", AstarVanguardPlugin.LogBuffer.GetUsedLayers().Select(kvp => $"{kvp.Key}({kvp.Value}s)"))).Append('\n')
                        .Append("- Used Reason:").Append('\n').Append(string.Join(", ", AstarVanguardPlugin.LogBuffer.GetUsedReasons().Select(kvp => $"{kvp.Key}({kvp.Value}s)"))).Append('\n')
                        .Append("- Vaild Trader Info: ").Append(GameLoop.Instance.Session.Traders.Any(t => t.Id == AstarVanguardPlugin.VanguardTraderId)).Append('\n')
                        .Append("- Total Exception: ").Append(AstarVanguardPlugin.LogBuffer.GetLogCount).Append("\n");

                    stringBuilder.Append("\n");
                    foreach (var logEntry in AstarVanguardPlugin.LogBuffer.GetEntries().Values)
                    {
                    ...
                    NotificationManagerClass.DisplayMessageNotification(string.Format(Locales.BIGSURVEYNOTIFY.McsLocalized(), AstarVanguardPlugin.LogBuffer.GetLogCount));
                    NewBigSurveyCount = 0;
