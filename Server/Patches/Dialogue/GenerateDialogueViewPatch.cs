...
namespace Astar.Vanguard.Server.Patches.Dialogue
{
    /// <summary>
    /// 对星锋好友首次获取聊天消息内容时都以商人消息类型发送一条问候语，保证玩家存档中与星锋的聊天类型一定以商人类型创建
    /// </summary>
    public sealed class GenerateDialogueViewPatch : AbstractPatch
    {
        ...
        {
            if (request.DialogId == TraderService.VanguardTraderId)
            {
                MailSendService.SendLocalisedNpcMessageToPlayer(
                    sessionId,
                    TraderService.VanguardTraderId,
                    MessageType.NpcTraderMessage,
                    Locales.VANGUARDTRADERWELCOME,
                    null
                );
