...
namespace Astar.Vanguard.Server.Patches.OrderQuest
{
    /// <summary>
    ...
    public sealed class CompleteQuestPatch : AbstractPatch
    {
        ...
        public static void Prefix(PmcData pmcData, CompleteQuestRequestData request, MongoId sessionID)
        {
            ...
            if (pmcData.TradersInfo.ContainsKey(Services.TraderService.VanguardTraderId))
            {
                return;
            }

            pmcData.TradersInfo[Services.TraderService.VanguardTraderId] = new TraderInfo
            {
                LoyaltyLevel = 1,
                SalesSum = 0.0,
                Standing = 0.0,
