...
using Astar.Vanguard.Server.ChatBot;
...
namespace Astar.Vanguard.Server.Patches.Friend
{
    /// <summary>
    ...
    public sealed class GetFriendListPatch : AbstractPatch
    {
        ...
        private static VanguardChatBot VanguardChatBot { get => field ??= ServiceLocator.ServiceProvider.GetService<VanguardChatBot>(); }
        ...
        {
            __result.Friends.Add(VanguardChatBot.GetChatBot());
            ...
            if (mcsBotPlayerProfiles is not null)
            {
                foreach (var mcsBotPlayerProfile in mcsBotPlayerProfiles)
                {
                    ...
                    var displayName = mcsPmcData.Info.Nickname;
